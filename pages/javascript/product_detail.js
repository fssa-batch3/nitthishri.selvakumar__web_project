...
function product_details(selected) {
    ...
    let buy_product=document.getElementById("buy_now")
    buy_product.setAttribute("href","../product/product_buynow.html?prod_id=" + selected["productid"])
}
...
let changing_star = document.querySelector(".review_star").value;

let reviewer_email = localStorage.getItem("logged_in");

let reviewer_matching = JSON.parse(localStorage.getItem("user_list"));

let reviews = JSON.parse(localStorage.getItem("customer_review")) || [];
let whole_box = document.querySelector(".testimonial");

let output_review = "";

let match = reviewer_matching.find((user) => user.emailid == reviewer_email);
